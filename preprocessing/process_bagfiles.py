...
def main(argv):
    ...
    dirname = args[0]
    dirnames = glob.glob(dirname + "normal_*")
    print dirnames
    #assert(False)
    for dirname in dirnames:
        ...
        for i, bag_filepath in enumerate(filelist):
            if not os.path.exists(bag_filepath.replace('bag','json')):
                ...
                proc = subprocess.Popen('rosrun pnpush_planning parse_bagfile_shapes.py %s --json %s' % (bag_filepath, rri) , shell=True)
                #proc = subprocess.Popen('rosrun pnpush_planning parse_bagfile_to_rawjson_2_circles.py %s --json %s' % (bag_filepath, rri) , shell=True)
